...
def main() -> None:
    if not PROCESSED_TABLE_PATH.exists():
        raise FileNotFoundError(
            f"Processed model table is missing: {PROCESSED_TABLE_PATH}. "
            "Please run src/02_prepare_data.py first."
        )

    REPORT_PATH.parent.mkdir(parents=True, exist_ok=True)
    print(f"[INFO] Loading processed data: {PROCESSED_TABLE_PATH}")
    df = pd.read_csv(PROCESSED_TABLE_PATH)
